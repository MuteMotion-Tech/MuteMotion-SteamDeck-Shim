import React, { VFC, useEffect, useRef, useCallback } from "react";
...
// Trail effect uses a position history buffer (not alpha-decay compositing,
// which causes permanent color accumulation due to 8-bit alpha rounding).
// =============================================================================
...
const GAIN = 18.0;             // raw IMU offset scaling (increased for sensitivity)
const MAX_TRAVEL = 70.0;       // max displacement in px (increased for more movement space)
...
export const DotGridView: VFC = () => {
        ...
        const poll = async () => {
            ...
            try {
                ...
                if (r && r.offset !== undefined && r.offset !== -88.8) {
                    ...
                    const rx = (r.offset_x || 0) * GAIN * inv;
                    const ry = (r.offset_y || 0) * GAIN * inv;
                    targetRef.current.x = Math.max(-MAX_TRAVEL, Math.min(MAX_TRAVEL, rx));
                    targetRef.current.y = Math.max(-MAX_TRAVEL, Math.min(MAX_TRAVEL, ry));
    ...
    const render = useCallback((now: number) => {
        ...
        // dynamic opacity based on motion magnitude
        const mag = Math.sqrt(target.x * target.x + target.y * target.y);
        const dynAlpha = Math.min(baseAlpha, 0.2 + (mag / MAX_TRAVEL) * (baseAlpha - 0.2));
    ...
    // mount render loop + initialize spring anchors
    useEffect(() => {
        const canvas = canvasRef.current;
        const w = canvas ? (canvas.clientWidth || 1280) : 1280;
        const h = canvas ? (canvas.clientHeight || 800) : 800;
        const leftX = w * MARGIN_PERCENT;
        const rightX = w * (1 - MARGIN_PERCENT);
        const springs = springsRef.current;

        for (let i = 0; i < DOT_COUNT; i++) {
            const y = h * (0.25 + (i * 0.5) / (DOT_COUNT - 1));
            springs[i].posX = leftX;
            springs[i].posY = y;
            springs[DOT_COUNT + i].posX = rightX;
